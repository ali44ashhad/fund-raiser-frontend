...
const PaymentManagement = () => {
  ...
  useEffect(() => {
    ...
    const fetchPayments = async () => {
      ...
      try {
        ...
        const data =
          (res && (Array.isArray(res) ? res : res?.data || res?.payments)) ||
          mockData;
  ...
  const getMethodBadge = (method) => {
    ...
    const baseClass =
      "px-2 py-1 text-xs font-medium bg-[#2A2A2A] text-[#FF7F11] rounded-full";
    if (m.includes("credit"))
      return <span className={baseClass}>Credit Card</span>;
