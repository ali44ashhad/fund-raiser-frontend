...
const Header = () => {
  ...
  // Normalize isAdmin to a boolean (covers string "true" or boolean true)
  const actuallyAdmin =
    Boolean(isAdmin) ||
    String(isAdmin) === "true" ||
  ...
  const handleLogout = () => {
    logout();
    // If admin logged out, route to admin login screen; otherwise home
    if (actuallyAdmin) {
  ...
  // While auth provider is initializing, render a minimal header to avoid flicker
  if (loading) {
    return (
      <header className="sticky top-0 z-50 bg-[#0B1D13] border-b border-[#4B5320] shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0B1D13] border-b border-[#4B5320] shadow-lg">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {currentUser ? (
              <div className="flex items-center space-x-4">
                ...
                {/* Only show "My Account" for non-admin users */}
                {!actuallyAdmin && (
                  <Link
                    to="/dashboard"
                    className="text-gray-300 hover:text-[#FF7F11] transition-colors"
      ...
      {isMenuOpen && (
        <div
          className="md:hidden bg-[#2A2A2A] border-t border-[#4B5320]"
          role="menu"
          aria-label="Mobile"
        >
          <div className="px-2 pt-2 pb-3 space-y-1">
